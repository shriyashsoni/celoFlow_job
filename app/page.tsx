...
import { connectWallet, formatAddress, WalletConnection, checkWalletConnection } from '@/lib/wallet';
import { ChevronRight, Zap } from 'lucide-react';
import { Footer } from '@/components/footer';

export default function Home() {
  const [role, setRole] = useState<'home' | 'employer' | 'employee'>('home');
  ...
  if (role === 'employer' && wallet) {
    return <EmployerDashboard address={wallet.address} onDisconnect={() => setWallet(null)} onBack={() => setRole('home')} />;
  }

  if (role === 'employee' && wallet) {
    return <EmployeeDashboard address={wallet.address} onDisconnect={() => setWallet(null)} onBack={() => setRole('home')} />;
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex justify-between items-center">
          ...
          {wallet ? (
            <div className="flex items-center gap-4">
              <div className="px-4 py-2 bg-[#0a0a0a] border border-[#1a1a1a] rounded-lg text-sm font-mono">
          ...
          {wallet ? (
            <div className="flex gap-4 justify-center flex-col sm:flex-row">
              ...
              <Button 
                ...
                size="lg"
                variant="outline"
                className="border-[#1a1a1a] hover:bg-[#0a0a0a] text-white font-bold"
              >
                I'm Receiving
              </Button>
            </div>
